# Сборка luci-app-olcrtc для OpenWRT

В этом документе описано, как собрать пакет `luci-app-olcrtc` (.apk) для OpenWRT 25+.
...
2. Скопируйте исходники пакета:
   ...
   ```bash
   cp -r /path/to/olcrtc-apk package/luci-app-olcrtc
   ```
...
3. Обновите фид и выберите пакет:
   ...
   `LuCI -> Applications -> <*> luci-app-olcrtc`

4. Соберите пакет:

   ```bash
   make package/luci-app-olcrtc/{clean,compile,install} V=s -j$(nproc)
   ```

5. Готовый `.apk` находится в:

   ```bash
   ls bin/packages/*/base/luci-app-olcrtc*.apk
   ```
...
1. В каталоге SDK добавьте в `feeds.conf.default`:
   ...
   ```
   src-link olcrtc /path/to/olcrtc-apk
   ```
...
2. Обновите и установите фид:
   ...
   ./scripts/feeds install luci-app-olcrtc
   ```

3. Соберите пакет:

   ```bash
   make package/luci-app-olcrtc/compile V=s -j$(nproc)
   ```
...
| `make package/luci-app-olcrtc/clean` | Очистить результаты сборки |
| `make package/luci-app-olcrtc/compile V=s` | Собрать пакет с подробным выводом |
| `make package/luci-app-olcrtc/install` | Установить в staging |
...
```bash
scp bin/packages/*/base/luci-app-olcrtc*.apk root@192.168.1.1:/tmp/
ssh root@192.168.1.1 opkg install /tmp/luci-app-olcrtc*.apk
```

## Зависимости

Пакет `luci-app-olcrtc` автоматически потянет следующие зависимости:
...
- Если `make menuconfig` не видит пакет, проверьте, что каталог `package/luci-app-olcrtc` содержит `Makefile`.
