...
NFT_DATA=$(nft list table inet olcrtc 2>/dev/null)
...
ol_rx=$(counter_bytes_nft "olcrtc_rx")
ol_tx=$(counter_bytes_nft "olcrtc_tx")
wd_rx=$(counter_bytes_nft "wdtt_rx")
wd_tx=$(counter_bytes_nft "wdtt_tx")

# In sing-box TUN mode the olcrtc counter comes from the TUN interface
...
if [ "$routing_core" = "sing-box" ] && [ "$proxy_mode" != "disabled" ]; then
	ol_rx=$(interface_bytes "tunolcrtc" "rx")
	ol_tx=$(interface_bytes "tunolcrtc" "tx")
fi
