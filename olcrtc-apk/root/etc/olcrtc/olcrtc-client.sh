...
CONFIG="olcrtc"
SECTION="${2:-olcrtc_main}"
BIN_DIR="/etc/olcrtc/bin"
PID_DIR="/var/run/olcrtc"
LOG_DIR="/var/log/olcrtc"
...
resolve_binary() {
	...
	case "$arch" in
		...
		x86_64)  [ -x "$BIN_DIR/olcrtc" ] && BIN_FILE="$BIN_DIR/olcrtc" ;;
	esac
	[ -z "$BIN_FILE" ] && BIN_FILE="$BIN_DIR/olcrtc"
}
...
pid_file() {
	echo "$PID_DIR/olcrtc_${SECTION}.pid"
}

log_file() {
	echo "$LOG_DIR/olcrtc_${SECTION}.log"
}
...
case "$1" in
	start)
		...
		[ -n "$2" ] && SECTION="$2"
		load_config
		[ "$(is_enabled)" -eq 1 ] || { echo "olcrtc not enabled"; exit 0; }
		resolve_binary
		[ -x "$BIN_FILE" ] || { echo "olcrtc binary not found ($BIN_FILE), run download first"; exit 1; }
		...
		if [ -f "$PID_FILE" ] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
			echo "olcrtc ($SECTION) already running"
			exit 0
		fi
