...
table inet $TABLE {
	...
	chain prerouting {
		...
		# olcrtc TCP redirect to SOCKS
		meta l4proto tcp ip daddr @proxy_ips counter name "olcrtc_rx" redirect to :$proxy_port
		meta l4proto tcp ip6 daddr @proxy_ips6 counter name "olcrtc_rx" redirect to :$proxy_port
	}
	...
	chain output {
		...
		# olcrtc TCP redirect to SOCKS for router-originated traffic
		meta l4proto tcp ip daddr @proxy_ips counter name "olcrtc_rx" redirect to :$proxy_port
		meta l4proto tcp ip6 daddr @proxy_ips6 counter name "olcrtc_rx" redirect to :$proxy_port
	}
...
EOF
	...
	if ! nft -c -f "$NFT_FILE"; then
		logger -t olcrtc "nftables rule validation failed"
		return 1
	fi

	if ! nft -f "$NFT_FILE"; then
		logger -t olcrtc "nftables setup failed"
		return 1
	fi
}
