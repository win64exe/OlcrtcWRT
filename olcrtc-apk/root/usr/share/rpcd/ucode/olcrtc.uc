#!/usr/bin/env ucode
'use strict';

import { readfile, popen, glob, stat } from 'fs';
import { cursor } from 'uci';

const CONFIG = 'olcrtc';
const PID_DIR = '/var/run/olcrtc';
const JSON = require('json');

function exec(cmd) {
	let f = popen(cmd);
	if (!f)
		return '';
	let out = f.read('all');
	if (out == null)
		out = '';
	f.close();
	return out;
}

function exec_json(cmd) {
	let str = exec(cmd);
	if (!str)
		return {};
	try {
		return JSON.parse(str) || {};
	} catch (e) {
		return {};
	}
}

function pid_running(pid) {
	if (!pid || pid <= 0)
		return false;
	return stat('/proc/' + pid) != null;
}

function pid_basename(path) {
	let parts = split(path, '/');
	let name = parts[length(parts) - 1];
	let len = length(name);
	if (len > 4 && substr(name, len - 4, 4) == '.pid')
		return substr(name, 0, len - 4);
	return name;
}

function get_nodes() {
	let nodes = {};
	let files = glob(PID_DIR + '/*.pid') || [];
	for (let i = 0; i < length(files); i++) {
		let path = files[i];
		let name = pid_basename(path);
		let pid = 0;
		let raw = readfile(path);
		if (raw != null) {
			let trimmed = trim(raw);
			if (trimmed != '')
				pid = int(trimmed);
		}
		if (pid_running(pid))
			nodes[name] = 'running';
		else
			nodes[name] = 'stopped';
	}
	return nodes;
}

function section_type(section) {
	let u = cursor();
	let type = u.get(CONFIG, section, 'type') || '';
	u.unload();
	return type;
}

function call_script(script, action, section) {
	return system('/etc/olcrtc/' + script + '-client.sh ' + action + " '" + section + "'");
}

function call_node_script(type, action, section) {
	let expected = section_type(section);
	if (expected != type)
		return { success: false, message: 'section type mismatch or missing' };

	if (type == 'olcrtc' || type == 'wdtt') {
		call_script(type, action, section);
		return { success: true };
	}

	return { success: false, message: 'unknown type' };
}

return {
	olcrtc: {
		status: {
			call: function(req) {
				return { nodes: get_nodes() };
			}
		},

		nodes: {
			call: function(req) {
				return { nodes: get_nodes() };
			}
		},

		start: {
			args: { type: '', section: '' },
			call: function(req) {
				return call_node_script(req.args.type, 'start', req.args.section);
			}
		},

		stop: {
			args: { type: '', section: '' },
			call: function(req) {
				return call_node_script(req.args.type, 'stop', req.args.section);
			}
		},

		restart: {
			args: { type: '', section: '' },
			call: function(req) {
				return call_node_script(req.args.type, 'restart', req.args.section);
			}
		},

		logs: {
			args: { type: '', lines: 50 },
			call: function(req) {
				let type = req.args.type;
				let lines = req.args.lines || 50;
				let out = '';
				if (type == 'olcrtc')
					out = exec('/etc/olcrtc/olcrtc-client.sh logs ' + lines);
				else if (type == 'wdtt')
					out = exec('/etc/olcrtc/wdtt-client.sh logs ' + lines);
				return out;
			}
		},

		ping: {
			call: function(req) {
				return exec_json('/etc/olcrtc/ping-monitor.sh');
			}
		},

		traffic: {
			call: function(req) {
				return exec_json('/etc/olcrtc/traffic-stats.sh');
			}
		},

		download: {
			call: function(req) {
				let rv = system('/etc/olcrtc/download-binaries.sh');
				return { success: (rv == 0) };
			}
		},

		nftables: {
			call: function(req) {
				let out = exec('nft list table inet olcrtc 2>/dev/null');
				return { ruleset: out };
			}
		},

		validate: {
			call: function(req) {
				let rv = system('/etc/olcrtc/nft-validate.sh >/dev/null 2>&1');
				return {
					valid: (rv == 0),
					message: (rv == 0) ? 'nftables rules are valid' : 'nftables rules are invalid or not generated'
				};
			}
		},

		update_subscription: {
			call: function(req) {
				let rv = system('/etc/olcrtc/subscription.sh update >/dev/null 2>&1');
				return {
					success: (rv == 0),
					message: (rv == 0) ? 'Subscription updated' : 'Subscription update failed'
				};
			}
		}
	}
};
