...
return view.extend({
	...
	render: function() {
		...
		o.datatype = 'port';
		o.depends('routing_core', 'nftables');
		o = s.option(form.Flag, 'redirect_to_olcrtc', _('Redirect proxy to olcrtc'));
		o.depends('routing_core', 'nftables');
		...
		node.appendChild(E('div', { 'class': 'cbi-section' }, [
			...
			E('p', {}, _('Download the latest olcrtc and WDTT binaries for this architecture.')),
			E('button', {
		...
		node.appendChild(E('div', { 'class': 'cbi-section' }, [
			...
			E('p', {}, _('Upload olcrtc or WDTT (proxy-turn-vk-android) binary manually. The file will be saved to /etc/olcrtc/bin/ and marked executable.')),
			E('div', { 'class': 'cbi-value' }, [
				E('label', { 'class': 'cbi-value-title' }, _('olcrtc binary')),
				E('div', { 'class': 'cbi-value-field' }, [
					E('input', {
						'type': 'file',
						'id': 'olcrtc-binary-file'
					})
				])
			]),
			E('div', { 'class': 'cbi-value' }, [
	...
	handleDownload: function(ev) {
		return ubus.call('olcrtc', 'download', {}).then(function() {
	...
	handleManualUpload: function(ev) {
		var olcrtcInput = document.getElementById('olcrtc-binary-file');
		var wdttInput = document.getElementById('wdtt-binary-file');
		var promises = [];

	if (olcrtcInput && olcrtcInput.files && olcrtcInput.files[0])
		promises.push(this.uploadFile('olcrtc', '/etc/olcrtc/bin/olcrtc', olcrtcInput.files[0], olcrtcInput));

	if (wdttInput && wdttInput.files && wdttInput.files[0])
		promises.push(this.uploadFile('WDTT', '/etc/olcrtc/bin/wdtt-server', wdttInput.files[0], wdttInput));
